...
if model_exist:
    ...
    df_train=df_sales.join(df_train_price_sum,competing_group,"inner").join(df_train_price_count,competing_group,"inner")
    df_train=df_train.withColumn("rl_price",col("price")*col("count")/col("price_sum")).withColumn("discount",(col("msrp")-col("price")) / col("msrp"))
    ...
    features_categorical_indexed_vec_index_train=columnindex(features_modeled_train,features_categorical_indexed_vec_train)
    ...
    df_train_modeling_rdd=df_train_modeling.map(lambda p: convert_sparsevec_to_vec_df(p,features_categorical_indexed_vec_index_train))
    df_train_modeling_rdd=df_train_modeling_rdd.map(lambda l: LabeledPoint(l[0],l[1:]))
    ################################################## 5: train random forest regression model
    ## random forest
    ## train model
    rfModel = RandomForest.trainRegressor(df_train_modeling_rdd, categoricalFeaturesInfo={},numTrees=100, featureSubsetStrategy="auto",impurity='variance', maxDepth=10, maxBins=32)
    ...
    df_model_performance=sqlContext.createDataFrame(sc.parallelize([[model_time,testMetrics.rootMeanSquaredError, testMetrics.r2]]),["model_time","RMSE","R2"])
    ...
    ## save the time file
    end_time=time.time()
    df_time=sqlContext.createDataFrame(sc.parallelize([["model_training",(end_time - start_time),model_time]]),                                       StructType([StructField("step", StringType(), True),StructField("time", DoubleType(), True),StructField("exec_date", StringType(), True)]))
