...
## scoring function
def scoring_df_batch(df,price_K,competing_group_vars, features_categorical_train_and_test, features_numerical_train_and_test, rfModel):
    ...
    price_single_range_df=[list(pr[0:len(competing_group_vars)])+[pr[-1]]+[i] for pr in price_range_df for i in np.linspace(pr[-3],pr[-2],price_K).tolist()]
    price_single_range_df=sqlContext.createDataFrame(price_single_range_df, competing_group_vars+["count","price"])
    price_sum_range_df=[list(pr[0:len(competing_group_vars)])+[i] for pr in price_range_df for i in np.linspace(pr[-1]*pr[-3],pr[-1]*pr[-2],(price_K-1)*pr[-1]+1).tolist()]
    ...
    features_categorical_indexed_vec_index_test=columnindex(features_modeled_test,features_categorical_indexed_vec_test)
    ...
    predictions = rfModel.predict(df_price_added_rdd                                  .map(lambda p: [p[i] for i in index_temp])                                  .map(lambda p: convert_sparsevec_to_vec_df(p,features_categorical_indexed_vec_index_test)))
...
def LPk(p_tmp):
    ...
    instance_reduce_value['whether_valid_price']=instance_reduce_value.apply(lambda row: 1 if row['price']>=row['cost'] and row['price']<=row['msrp'] else 0,axis=1)
    for i in range(products_num):
        whether_valid_price_individual_product=instance_reduce_value['whether_valid_price'].iloc[price_num*i:price_num*(i+1)-1]
        if all(whether_valid_price== 0 for whether_valid_price in whether_valid_price_individual_product):
            abs_diff_price=abs(instance_reduce_value['msrp'].iloc[price_num*i:price_num*(i+1)]-instance_reduce_value['price'].iloc[price_num*i:price_num*(i+1)])
    ...
    ## set constrains 1
    for i in range(products_num):   
        cons_tmp=[1 if (i_tmp>=price_num*i) & (i_tmp<price_num*(i+1)) else 0 for i_tmp in range(len(instance_reduce_value))]
...
def IPk(ps_tmp):
    ...
    ## get whether_valid_price, if equal to 1, valid, if equal to 0, invalid
    ## invalid ones needed to set into the constrains
    obj_mat['whether_valid_price']=obj_mat.apply(lambda row: 1 if row['price']>=row['cost'] and row['price']<=row['msrp'] else 0,axis=1)
    for i in range(products_num):
        whether_valid_price_individual_product=obj_mat['whether_valid_price'].iloc[price_num*i:price_num*(i+1)-1]
        if all(whether_valid_price== 0 for whether_valid_price in whether_valid_price_individual_product):
            abs_diff_price=abs(obj_mat['msrp'].iloc[price_num*i:price_num*(i+1)]-obj_mat['price'].iloc[price_num*i:price_num*(i+1)])
            row_index=i*price_num+min(enumerate(abs_diff_price),key=lambda x: x[1])[0]
    ...
    ## set constrains 1
    for i in range(products_num):   
        cons_tmp=[1 if (i_tmp>=price_num*i) & (i_tmp<price_num*(i+1)) else 0 for i_tmp in range(len(obj_mat))]
        ret = lpsolve('add_constraint', lp, cons_tmp, EQ, 1)
    ## set constrains 2
...
df_test=df_test.select(['store_id', 'product_id','department_id', 'brand_id', 'msrp', 'cost', 'avg_hhi', 'avg_traffic','week_start']).distinct()


# In[11]:

################################################## 3: load the model built last time
dirfilename_load= sqlContext.read.parquet(modelDir+"model_name").map(lambda p:p).filter(lambda p:p[0]=="model_name").map(lambda p: p[1]).collect()
...
indexer=StringIndexer(inputCol=competing_group_index_column_name,outputCol=competing_group_index_column_name_StringIndexed)
...
for batch_size_index in range(len(batch_size_index_starts)-1):
    ...
    df_batch=df.filter(competing_group_index_column_name_StringIndexed+">="+str(batch_size_index_starts[batch_size_index]))                .filter(competing_group_index_column_name_StringIndexed+"<"+str(batch_size_index_starts[batch_size_index+1]))
    df_batch_scored, df_batch_scored_names=scoring_df_batch(df_batch,price_K,competing_group_vars,                                        features_categorical_train_and_test, features_numerical_train_and_test, rfModel)
    ...
    df_batch_scored_reduced=df_batch_scored.map(lambda p: reduce_key_value_map_LPk(p,index_reduce_key_vars,index_reduce_value_vars)).reduceByKey(lambda a, b: a + b)
    ...
    df_batch_scored_reduced_filter_condition=df_batch_scored_reduced.map(lambda p: '_'.join([str(item) for item in p])).collect()
    df_batch_scored_pred_demand=df_batch_scored.map(create_filter_condition)                                               .filter(lambda p: p[0] in df_batch_scored_reduced_filter_condition)                                               .map(lambda p: p[1])
    ...
    df_batch_scored_pred_demand_names_needed_names=[item for i,item in enumerate(df_batch_scored_pred_demand_names) if 'StringIndexed' not in item]
    df_batch_scored_pred_demand_names_needed_index=[i for i,item in enumerate(df_batch_scored_pred_demand_names) if 'StringIndexed' not in item]
    df_batch_scored_pred_demand_needed=df_batch_scored_pred_demand.map(lambda p: [p[i] for i in df_batch_scored_pred_demand_names_needed_index]).collect()
...
################################################## 4: output the data
dir_exists=subprocess.call(["hadoop", "fs", "-test", "-d", opt_results_d_loc+'week_start='+processed_time_d[1].strftime('%Y-%m-%d')])
if dir_exists==1:
    ...
    price_change_d=sc.parallelize([[p[item].strftime('%Y-%m-%d %H:%M:%S') if item==df_batch_scored_pred_demand_names_needed_names.index('week_start') else p[item] for i,item in enumerate(price_change_names_index)] for p in df_final_output])
    price_change_d=price_change_d.map(lambda p: ','.join(str(item) for item in p))
    dir_exists=subprocess.call(["hadoop", "fs", "-test", "-d", price_change_d_loc])
    if(dir_exists==0):
        subprocess.call(["hadoop", "fs", "-rm", "-r","-skipTrash",price_change_d_loc])
    price_change_d.repartition(1).saveAsTextFile(price_change_d_loc)
    ## output the optimization results
    sqlContext.createDataFrame(sc.parallelize(df_final_output),df_batch_scored_pred_demand_names_needed_names).write.partitionBy('week_start').parquet(opt_results_d_loc, mode='append')
    ## output the time
    end_time=time.time()
    agg_time=processed_time_d[1]
    df_time=sqlContext.createDataFrame(sc.parallelize([["optimization",(end_time - start_time),str(agg_time-timedelta(7))]]),                                       StructType([StructField("step", StringType(), True),StructField("time", DoubleType(), True),StructField("exec_date", StringType(), True)]))
