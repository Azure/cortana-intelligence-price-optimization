...
def scoring_df_batch(df, price_K, competing_group_vars, features_categorical_train_and_test,
    ...
    price_single_range_df = spark.createDataFrame(price_single_range_df, competing_group_vars + ["count", "price"])
    ...
    price_sum_range_df = spark.createDataFrame(price_sum_range_df, competing_group_vars + ["price_sum"])
...
## input paths
df_train_loc = "wasb://{1}@{0}.blob.core.windows.net/".format(blob_storage_name,result_container) + "train_data/"
...
modelDir = "wasb://{1}@{0}.blob.core.windows.net/".format(blob_storage_name,result_container) + "Models/"
## output paths
price_change_d_loc = "wasb://{1}@{0}.blob.core.windows.net/".format(blob_storage_name,result_container) + "medium_results/suggested_prices"
opt_results_d_loc = "wasb://{1}@{0}.blob.core.windows.net/".format(blob_storage_name,result_container) + "opt_results_data/"
...
df_train = spark.read.parquet(df_train_loc)
df_train = df_train.select(['week_start', 'store_id', 'department_id', 'product_id', 'sales', 'date', 'price', 'brand_id', 'msrp', 'cost', 'avg_hhi', 'avg_traffic', 'group_val', 'rl_price', 'discount'])
df_sales = spark.read.parquet(df_sales_loc)
...
dirfilename_load = spark.read.parquet(modelDir + "model_name").filter('_1 == "model_name"').rdd.map(lambda p: p[1]).collect()
...
if dir_exists == 1:
    ...
    df_opt = spark.createDataFrame(sc.parallelize(df_final_output), df_batch_scored_pred_demand_names_needed_names)
    ...
    #df_time = spark.read.parquet(df_time_hive_loc)
    ...
    df_time_all = spark.createDataFrame(
